#!/usr/bin/env python3
"""
Script para importar usuários do CSV para o Supabase (versão simplificada)
Uso: python scripts/import_users_simple.py usuarios.csv --auto
"""
...
class SupabaseImporter:
    ...
    def create_user_simple(self, email, password, metadata=None):
        """Cria um usuário usando apenas PUT à tabela auth.users (sem trigger)"""
        ...
        try:
            ...
            raise Exception("Timeout ao conectar com o Supabase")
        
        if response.status_code not in [200, 201]:
            error_data = response.json() if response.headers.get('content-type') == 'application/json' else response.text
            
            # Se for erro 500 "Database error creating new user", pode ser um trigger
            if isinstance(error_data, dict) and error_data.get('error_code') == 'unexpected_failure':
                raise Exception(f"Erro de banco de dados (verifique trigger em profiles): {error_data.get('msg')}")
            ...
            try:
                with open(filepath, 'r', encoding=encoding) as f:
                    ...
                    for i, row in enumerate(reader, start=2):
                        ...
                        # Validar email (não permitir múltiplos)
    ...
    def import_users(self, users):
        ...
        for i, user in enumerate(users, start=1):
            ...
            try:
                ...
                self.create_user_simple(user['email'], user['password'], metadata)
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print("Uso: python scripts/import_users_simple.py usuarios.csv [--auto]")
        sys.exit(1)
    ...
    try:
        ...
        if result['errors'] > 0:
            ...
            for error in result['details'][:5]:  # Mostrar apenas 5 primeiros erros
                print(f"   - {error['email']}: {error['error']}")
            if len(result['details']) > 5:
                print(f"   ... e mais {len(result['details']) - 5} erros")

        if result['errors'] == 0:
            print("\n🎉 Importação concluída com sucesso!")
        else:
            print("\n⚠️  Importação concluída com alguns erros")
            print("\n💡 Dica: Se o erro é 'Database error creating new user', execute:")
            print("   1. Acesse SQL Editor no Supabase")
            print("   2. Execute: scripts/fix-profiles-trigger.sql")
